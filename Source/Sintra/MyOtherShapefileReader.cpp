...
#include "MyOtherShapefileReader.h"

TArray<FLine> UMyOtherShapefileReader::getSplinePoints()
{
	...
	/* Shapefile Setup */
	char* shapeFile = "D:/Daniel_Casadinho/New_Data/Vetor/L_hidrografia_3D_other.shp";
	GDALAllRegister();
	...
	OGRLayer  *pLayer = road->GetLayerByName("L_hidrografia_3D_other");
...
double UMyOtherShapefileReader::getElevation(double pixelX, double pixelY, GDALRasterBand *demBand, float *scanLine) {
	demBand->RasterIO(GF_Read, pixelX, pixelY, 1, 1, scanLine, 1, 1, GDT_Float32, 0, 0);

	return *scanLine;
}

double UMyOtherShapefileReader::getWorldX(double geoX)
{
	return geoX * 500;
}

double UMyOtherShapefileReader::getWorldY(double geoY)
{
	return geoY * 500;
}
